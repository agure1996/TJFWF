...
export default function Expenses() {
  ...
  const mapExpenseType = (value: string): ExpenseType["expenseType"] => {
    const validTypes: KnownExpenseType[] = Object.keys(
      TYPE_COLORS,
    ) as KnownExpenseType[];
    return validTypes.includes(value as KnownExpenseType)
      ? (value as KnownExpenseType)
      : "OTHER";
  };
  ...
  const getBadgeColor = (type: string) => {
    ...
    return darkMode
      ? TYPE_COLORS_DARK[typeKey] || TYPE_COLORS_DARK.OTHER
    ...
    <div className="px-4 sm:px-6 lg:px-8 py-6">
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div
            className={`hidden md:block rounded-xl shadow-sm overflow-x-auto ${
              darkMode
                  ...
                  >
                    <td className="px-4 py-3 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        >
                          <Wallet
                            className={`w-4 h-4 ${darkMode ? "text-rose-400" : "text-rose-600"}`}
                          />
                        </div>
                        <div
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <Badge
                        className={`text-xs ${getBadgeColor(expense.expenseType)}`}
                      >
                        {expense.expenseType}
                      </Badge>
                    </td>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <div
                        className={`text-sm font-semibold ${darkMode ? "text-rose-400" : "text-rose-600"}`}
                      >
                    ...
                    <td className="px-4 py-3">
                      <div
                        className={`text-sm truncate max-w-xs ${darkMode ? "text-[#A39180]" : "text-slate-500"}`}
                      >
                        {expense.notes || "—"}
                      </div>
                    </td>
              ...
              >
                ...
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-3 flex-1">
                    ...
                    >
                      <Wallet
                        className={`w-5 h-5 ${darkMode ? "text-rose-400" : "text-rose-600"}`}
                      />
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <Badge
                        className={`text-xs mt-1 ${getBadgeColor(expense.expenseType)}`}
                      >
                        {expense.expenseType}
                      </Badge>
                    </div>
                  ...
                  {expense.notes && (
                    <div
                      className={`text-xs pt-2 border-t ${
                        darkMode
                          ? "border-neutral-700 text-[#A39180]"
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent
          className={`w-full sm:max-w-md max-h-[90vh] sm:rounded-lg overflow-y-auto ${
