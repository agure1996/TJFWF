...
export default function Sales(): JSX.Element {
  ...
  const { toastCreate, toastError, toastUpdate, toastDelete } =
    useToastHelper();
    ...
    if (location.state?.highlightSaleId) {
      ...
      setTimeout(() => {
        const element = document.getElementById(
          `sale-${location.state.highlightSaleId}`,
        );
        if (element) {
  ...
  const handleCreate = async (payload: CreateSaleRequest) => {
    try {
      if (editingSale) {
        ...
        toastUpdate("Sale updated successfully");
      } else {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      toastDelete("Sale deleted successfully");
      fetchSales();
    } catch (error) {
      toastError(error);
    }
                      ...
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          {sale.customerContact && (
                            <div
                              className={`text-xs ${darkMode ? "text-[#A39180]" : "text-slate-500"}`}
                            >
                              {sale.customerContact}
                            </div>
                          )}
